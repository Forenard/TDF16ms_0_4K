...
float CHORDS[] = float[](0.0, 2.0, 3.0, 7.0, 14.0, 15.0, 19.0, 22.0, -5.0, 2.0, 3.0, 5.0, 7.0, 10.0, 14.0, 15.0, -4.0, 0.0, 3.0, 7.0, 14.0, 15.0, 19.0, 24.0, -7.0, 0.0, 7.0, 8.0, 10.0, 12.0, 15.0, 19.0);
...
void main()
{
  ...
  { // perc
    for(int i = 0; i < 32; i++)
    {
      ...
      vec2 i_wave = fbm32(vec2(exp2(-2.0 + 4.0 * dice2.y) * orbit(phase) + exp2(-2.0 + 5.0 * dice2.z) * phase)).xy - 0.5;
  ...
  { // arp
    for(int i = 0; i < 64; i++)
    {
      ...
      vec2 phase = (t * freq + pcg33(vec3(i_fi)).xy + 0.4 * fbm32(vec2(0.1 * t * freq)).xy);
