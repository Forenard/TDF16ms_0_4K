...
"#version 430\n"
 ...
 "{"
   ...
   "float e=clamp(dot(c,l),0.,1.)+1e-5,d=clamp(dot(c,f),0.,1.),n=clamp(dot(c,s),0.,1.),u=clamp(dot(f,s),0.,1.),t=n*a,i=a/(1.-n*n+t*t),o=a*a,b=.5+2.*a*u*u;"
   "return(i*i/x*(.5/(d*sqrt(max(0.,e*e*(1.-o)+o))+e*sqrt(max(0.,d*d*(1.-o)+o))))*(z+(1.-z)*r(1.-u))+v*(1.-y)*((1.+(b-1.)*r(1.-d))*(1.+(b-1.)*r(1.-e))/x))*d;"
 "}"
 ...
 "{"
   ...
   "for(int i=0;i<256;i++)"
     "{"
       ...
       "vec2 a=floor(f.xy/vec2(3,2)+sign(v.xy))*vec2(3,2)+vec2(1.5,1),b=abs(a-f.xy)-vec2(1.5,1);"
       "b=max(b,0.)/abs(v.xy)+.001+step(f.z,-30)*1e2;"
       "float u=min(b.x,b.y);"
       "c=min(u,c);"
       "r+=c;"
       "if(c<.001)"
 ...
 "{"
   ...
   "u=normalize(mix(vec3[](vec3(.5,1.5,1),vec3(-.5,-1,-1),vec3(-1,-.1,.3),vec3(.8,.8,1),vec3(0,0,1),vec3(-.3,.5,1),vec3(-1,.2,0),vec3(1,1,0),vec3(0,0,1))[B],vec3[](vec3(.1,.1,1),vec3(0,.5,-1),vec3(-1,-.1,1),vec3(.1,.8,1.5),vec3(0,0,1),vec3(.3,.5,1),vec3(-1,-.2,0),vec3(0,1,.1),vec3(0,0,1))[B],C));"
   "const vec3 A=normalize(cross(vec3(0,1,0),u));"
   "o=normalize(mat3(A,normalize(cross(u,A)),u)*vec3((b-.5+(p.xy-.5)*.25/z)*2.*(z/min(z.x,z.y)),1./tan(70.*x/360.)));"
   "vec3 D,E;"
