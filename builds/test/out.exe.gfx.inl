...
"#version 430\n"
 ...
 "const vec3 d=vec3(255,137,14)/255.,a=vec3(191,211,255)/255.;"
 "vec2 t(float v)"
 "{"
 ...
 "{"
   ...
   "f=f*f*(3.-2.*f);"
   "vec3 c=mix(r(vec3(m,.12)),r(vec3(m+vec2(1,0),.12)),f.x),y=mix(r(vec3(m+vec2(0,1),.12)),r(vec3(m+vec2(1),.12)),f.x);"
   "return mix(c,y,f.y);"
 "}"
 "vec3 h(vec2 v)"
 "{"
 ...
 "{"
   ...
   "float s=clamp(dot(c,z),0.,1.)+1e-5,u=clamp(dot(c,f),0.,1.),n=clamp(dot(c,a),0.,1.),o=clamp(dot(f,a),0.,1.),p=n*l,r=l/(1.-n*n+p*p),g=l*l,i=.5+2.*l*o*o;"
   "return(r*r/x*(.5/(u*sqrt(max(0.,s*s*(1.-g)+g))+s*sqrt(max(0.,u*u*(1.-g)+g))))*(d+(1.-d)*w(1.-o))+v*(1.-y)*((1.+(i-1.)*w(1.-u))*(1.+(i-1.)*w(1.-s))/x))*u;"
 "}"
 ...
 "{"
   ...
   "z=v-vec3(1.15,-.76,0);"
   "a=e(z,vec3(.4,.3,.15)-.02);"
   "z-=vec3(-.06,0,-.11);"
   "a=max(a,-e(z,vec3(0,0,.1))+.1);"
   "z.z-=.05;"
   "z.y-=.01*clamp(round(z.y/.01),-9,9);"
   "a=min(a,e(z,vec3(.2,0,0))-.002);"
   "b=min(b,a);"
   "float g=n?"
     "o:"
   ...
   "h.z-=1.+B*w;"
   "a=e(h,vec3(2.6,1.6,.01))*.5-.01;"
   "a+=float(n)*1e9;"
   "c=a<g?"
     "1:"
     "0;"
   "g=min(g,a);"
   "i=length(v-vec3(sign(f.x-.5),-.09,0))-.1;"
   ...
   "for(int i=0;i<256;i++)"
     "{"
       ...
       "f.xz=105.<=y&&y<120.?"
         ...
         "f.xz;"
       "float l=n/.1171875/4+floor(f.x/6);"
       "f.y+=3.*(floor(l/4.)-pow(1./(1.+mod(l,4.)),5.))*sign(fract(f.x/12)-.5)*step(75.,y);"
       "c=u(f);"
       "float o=c*c/(2.*a),g=sqrt(c*c-o*o);"
       "z=min(z,g/(.01*max(0.,b-o)));"
       "a=c;"
