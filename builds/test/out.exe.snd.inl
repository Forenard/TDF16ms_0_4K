...
"#version 430\n"
 ...
 "{"
   ...
   "vec4 l=vec4(f%(5625*ivec4(1,4,16,256)))/48000;"
   "float y=f/48000/16/x,r;"
   "vec2 z=vec2(0);"
   "float u;"
   "r=l.y;"
   "u=y>=16?"
   ...
   "if(y>=8)"
     "{"
       ...
       "{"
         ...
         "z+=.4*u*c*tanh(4*(f%22500/48000)*sin(v*g));"
       "}"
       "for(int g=0;g<8;g++)"
         "{"
           "vec3 D=p(vec3(g+51));"
           "float n=exp2((30+w[_]+.2*(sqrt(-2*log(D.xy.x))*t(v*D.xy.y)).x-69)/12)*440*r+D.z;"
           "z+=.1*u*tanh(4*(f%22500/48000)*c*tanh(3*((fract(n)+smoothstep(1.,0.,fract(n)/(.55-.4*exp(-r*5))))*2-1-.55+.4*exp(-r*5))));"
         "}"
     "}"
   "if(y>=16)"
     "{"
